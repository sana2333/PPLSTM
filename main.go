...
func lstm(dataName string, batchID int) {
	...
	if _, err := file.WriteString(fmt.Sprintf("%s_%d elapsed time: %v\n", dataName, batchID, elapsed)); err != nil {
		fmt.Println("writen file error:", err)
	}
}
